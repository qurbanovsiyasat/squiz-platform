-- Fix Core Admin and Q&A Issues
-- Date: 2025-08-09
-- Fixes: Focus on essential fixes only

-- 1. DROP AND RECREATE ESSENTIAL FUNCTIONS ONLY
...
-- 2. CREATE CORRECTED get_all_users_with_admin_info FUNCTION
...
-- 3. ENHANCED CATEGORY FUNCTION (without conflicts)
CREATE OR REPLACE FUNCTION get_categories_by_type_safe(p_category_type TEXT)
RETURNS TABLE (
    id UUID,
    name TEXT,
    type TEXT,
...
GRANT EXECUTE ON FUNCTION get_categories_by_type_safe(TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION get_form_stats(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION get_form_stats(UUID) TO anon;

RAISE NOTICE 'Core admin issues migration completed successfully';
